...
export class SubscriptionManager {
    constructor(env, { rpc, orm, notification }) {
        ...
        if (session.expiration_date) {
            ...
            this.expirationDate = DateTime.utc().plus({ days: 90 });
        }
    ...
    hideWarning() {
        ...
        cookie.set("oe_instance_hide_panel", true, 24 * 60 * 6000);
        this.isWarningHidden = true;
    }

    async buy() {
        const limitDate = serializeDate(DateTime.utc().minus({ days: 90 }));
        const args = [
            [
                ["share", "=", false],
